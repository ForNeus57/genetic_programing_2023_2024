...
/**
 ...
 input: (X1  * ((( 3.2 + 3.4) / ( 3.3 + 4.3)) * 2)) output: (X1  * 2)
...
public class Simplifier {
    ...
    /**
     * Move through the whole program string, and count any remaining sub formulas that
     * can be simplified (details). Sub-formulas that can be simplified contain no X#
     * variables.
     *
     * @return An integer showing the number of sub formulas that can be
    ...
    public void locateAvailableSubFormula() {
        ...
        for (int i = 0; i < this.newProg.length(); i++) { // for loop moves through program string
            ...
            if ( isSimplifiableSubFormula(iFirstParen, iSecondParen) ) { // can this subformula be simplified?
                ...
                return;                         // since subformula found, return location of first parenthesis
            }
